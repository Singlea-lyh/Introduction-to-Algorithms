...
*
*算法描述：采用归并排序的思想，在最后合并时，比较数组中元素大小，计算逆序对数，每次合并时，前面数组元素小于后面数组，说明L中后续元素都比R中后续元素大，此时逆序对数加上L中i之后还剩元素个数
...
*/ 
#include <iostream>
#define MAX_VALUE 12345678
using namespace std;
#if 1             // 定义全局变量完成统计
int globalInversion = 0;
void  INVERSIONMERGER(int A[], int left, int mid, int right)
{
	int i, j, k;
	int n1 = mid - left + 1;
	int n2 = right - mid;
...
void  MERGERSORT(int A[], int left, int right)
{
	...
	INVERSIONMERGER(A, left, mid, right); 
/*	
    for(int i = 0; i < 5; ++i)
	  cout<< A[i] << " ";
	cout<<endl;
